...
exports.list = function(callback) {
  return childProcess.exec('wmic logicaldisk get DeviceID, VolumeName, Size', {}, function(error, stdout, stderr) {
    var result;
    if (error != null) {
    ...
    result = _.map(result, function(row) {
     ...
     //   row.Caption = row.VolumeName.concat(_.initial(row.DeviceID));
     // }
		row.VolumeName = row.VolumeName +"";
      return {
        device: (_.first(row.DeviceID)) + "\\",
		description: (row.VolumeName).replace(',', ' '),
