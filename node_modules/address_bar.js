...
var gen_bar_win32 = jade.compile([
    '- each item, i in sequence',
    '  - if (i != sequence.length - 1)',
    '    li(data-path="#{item.path}")',
    '      a(href="#") #{item.name}',
...
AddressBar.prototype.set = function(dir_path) {
  ...
  if (process.platform != 'win32'){
	...
	sequence = this.current_path.split(path.sep);
  ...
  if (sequence[0] == '') {
    result[0] = {
	  ...
	  var caption = this.current_path.split(":\\");
	  ...
	  if (caption[1].length!=0){
		sequence = caption[1].split(path.sep);
 
		for (; i < sequence.length; ++i) {
		result.push({
		name: sequence[i],
		path:  caption[0]+":\\" + sequence.slice(0, 1 + i).join(path.sep),
		});
		}
	  }
	  this.element.html(gen_bar_win32({ sequence: result})); 
  }
